...
// نقطة البداية: موقع الزائر — دائرة زرقاء نابضة (Pulse) مطابقة للتصميم المرجعي
const buildUserIcon = (moving: boolean) =>
  L.divIcon({
    className: 'soufmap-user-icon',
    html: moving
      ? `<div class="soufmap-user-pulse"></div><div class="soufmap-user-car">🚗</div>`
      : `<div class="soufmap-user-pulse"></div><div class="soufmap-user-dot"></div>`,
    iconSize: [44, 44],
    iconAnchor: [22, 22],
  });

// نقطة النهاية: المعلم — دبوس أحمر (Pin) مطابق للتصميم المرجعي
const destinationIcon = L.divIcon({
  className: 'soufmap-dest-icon',
  html: `<div class="soufmap-pin"><div class="soufmap-pin-dot"></div></div><div class="soufmap-pin-anchor"></div>`,
  iconSize: [34, 46],
  iconAnchor: [17, 44],
});
...
function FollowOrFit({
  ...
  useEffect(() => {
    ...
    if (!isNavigating && routeCoordinates.length > 1) {
      ...
      map.fitBounds(bounds, { paddingTopLeft: [60, 140], paddingBottomRight: [60, 220] });
      return;
    }
    if (selectedPlace) {
...
function MapControls({
  ...
  return (
    <div ref={wrapperRef} className="soufmap-controls">
      <button type="button" onClick={handleReorient} aria-label="إعادة توجيه الخريطة" className="soufmap-ctrl-btn">
        <Compass size={18} />
      </button>
      <div className="soufmap-ctrl-group">
        <button type="button" onClick={() => map.zoomIn()} aria-label="تكبير" className="soufmap-ctrl-btn">
          <Plus size={18} />
        </button>
        <button type="button" onClick={() => map.zoomOut()} aria-label="تصغير" className="soufmap-ctrl-btn">
          <Minus size={18} />
        </button>
      </div>
      <button type="button" onClick={onLocateUser} aria-label="تحديد موقعي الحالي" className="soufmap-ctrl-btn">
        <LocateFixed size={18} />
      </button>
      <button type="button" onClick={handleFullscreen} aria-label="ملء الشاشة" className="soufmap-ctrl-btn">
